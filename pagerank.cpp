...
struct Page
{
	vector<ID_TYPE> Links;//外链,所有的外链ID
	ID_TYPE src;//自身ID用于排序
...
*	Pages* 所有页，长度为max_id
*/
Page* Init(const char* file_name, ID_TYPE& max_id)
{
	/*边(链接)的数据类型from->to*/
	struct Border { ID_TYPE from, to; };
	ifstream input;
	ID_TYPE from, to, count = 0;
...
void PageRank(const char* file_name, RANK_TYPE beta = 0.85)
{
	...
	/*结果写入文件*/
	ofstream out;
	out.open("pagerank.txt");
	for (unsigned int i = 0; i < max_id; i++)
	{
		out << pages[i].src << "\t" << setprecision(12) << pages[i].Rank << endl;
	}
	out.close();
	cout << "全部" << max_id << "条结果已输出到 pagerank.txt中\n";
	TIME_COUNT("排序输出")/*记时*/
}
...
{
	const char* file_name = argc < 2 ? "WikiData.txt" : argv[1];//第一个参数文件
